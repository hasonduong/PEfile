...
void PE64FILE::PrintImportTableInfo() {
	...
	for (int i = 0; i < _import_directory_count; i++) {
		...
		printf("       ILT RVA: 0x%X\n", PEFILE_IMPORT_TABLE[i].HT.OriginalFirstThunk);
		...
		DWORD ILTAddr = resolve(PEFILE_IMPORT_TABLE[i].HT.OriginalFirstThunk, locate(PEFILE_IMPORT_TABLE[i].HT.OriginalFirstThunk));
		int entrycounter = 0;
